...
import { CallToolRequestSchema, ErrorCode, McpError, ListToolsRequestSchema, ToolSchema, } from "@modelcontextprotocol/sdk/types.js";
import { z } from "zod";
...
class ParameterHandler {
    ...
    // 统一处理detailed参数
    static normalizeDetailed(detailed) {
        return detailed ?? false;
    }
    // 统一处理limit参数
    static normalizeLimit(limit, defaultLimit = 20) {
        return limit || defaultLimit;
    }
    // 统一处理format参数
    static normalizeFormat(format, defaultFormat = 'text') {
...
class ErrorHandler {
     ...
     * 安全执行异步操作
     */
    static async safeExecute(operation, context, fallback) {
        try {
...
class SchaleDBClient {
    ...
    constructor(config = {}) {
        ...
        this.cache = new Cache(this.config.cacheTimeout / (60 * 1000));
    }
    ...
    // 智能搜索算法 - 支持模糊匹配和多语言
    smartSearch(items, searchTerm, fields) {
        if (!searchTerm)
            return items;
    ...
    // 数据字段精简
    simplifyStudentData(student, detailed = false) {
        if (detailed) {
            ...
            if (student.Skills && Array.isArray(student.Skills)) {
                result.SkillsFormatted = student.Skills.map((skill, index) => ({
                    SkillIndex: index + 1,
    ...
    // 计算单个属性的成长曲线
    calculateStatCurve(level1Value, level100Value) {
        const curve = [];
    ...
    simplifyRaidData(raid, detailed = false) {
        if (detailed) {
            return raid;
        }
        return {
    ...
    simplifyEquipmentData(equipment, detailed = false) {
        if (detailed) {
            return equipment;
        }
        return {
    ...
    simplifyStageData(stage, detailed = false) {
        if (detailed) {
            return stage;
        }
        return {
    ...
    simplifyItemData(item, detailed = false) {
        if (detailed) {
            return item;
        }
        return {
    ...
    simplifyFurnitureData(furniture, detailed = false) {
        if (detailed) {
            return furniture;
        }
        return {
    ...
    simplifyEnemyData(enemy, detailed = false) {
        if (!detailed) {
            return {
                Id: enemy.Id,
                Name: enemy.Name,
    ...
    async getStudents(options = {}) {
        const { language = 'cn', search, limit = 20, detailed = false, school, starGrade, role, compressed = false } = options;
        ...
        // 按学校过滤
        if (school) {
            filteredStudents = filteredStudents.filter((s) => s.School && s.School.toLowerCase().includes(school.toLowerCase()));
        }
        // 按星级过滤
        if (starGrade) {
        ...
        // 按职业过滤
        if (role) {
            filteredStudents = filteredStudents.filter((s) => s.TacticRole && s.TacticRole.toLowerCase().includes(role.toLowerCase()));
        }
        // 智能搜索
        if (search) {
            filteredStudents = this.smartSearch(filteredStudents, search, ['Name', 'School', 'TacticRole', 'WeaponType']);
        }
        // 限制结果数量
    ...
    // 通过名称查询学生
    async getStudentByName(name, language = 'cn', detailed = false) {
    ...
    async getRaidsEnhanced(language = 'cn', detailed = false) {
    ...
    async getEquipment(language = 'cn') {
    ...
    async getStagesEnhanced(options = {}) {
        ...
        // 按区域筛选
        if (area) {
            filteredStages = filteredStages.filter(s => s.Area && s.Area.toLowerCase().includes(area.toLowerCase()));
        }
        // 按章节筛选
        if (chapter) {
            filteredStages = filteredStages.filter(s => s.Chapter && s.Chapter.toLowerCase().includes(chapter.toLowerCase()));
        }
        // 按难度筛选
        if (difficulty) {
            filteredStages = filteredStages.filter(s => s.Difficulty && s.Difficulty.toLowerCase().includes(difficulty.toLowerCase()));
        }
        // 智能搜索
        if (search) {
            filteredStages = this.smartSearch(filteredStages, search, ['Name', 'Area', 'Chapter', 'Difficulty']);
        }
        // 限制结果数量
    ...
    async getItemsEnhanced(options = {}) {
        const { language = 'cn', search, category, rarity, tags, limit = 20, detailed = false } = options;
        ...
        // 按类别筛选
        if (category) {
            filteredItems = filteredItems.filter(i => i.Category && i.Category.toLowerCase().includes(category.toLowerCase()));
        }
        // 按稀有度筛选
        if (rarity !== undefined) {
        ...
        // 按标签筛选
        if (tags) {
            filteredItems = filteredItems.filter(i => i.Tags && i.Tags.some(tag => tag.toLowerCase().includes(tags.toLowerCase())));
        }
        // 智能搜索
        if (search) {
            filteredItems = this.smartSearch(filteredItems, search, ['Name', 'Category', 'Tags', 'Description']);
        }
        // 限制结果数量
    ...
    async getFurnitureEnhanced(options = {}) {
        const { language = 'cn', search, category, type, rarity, tags, limit = 20, detailed = false } = options;
        ...
        // 按类别筛选
        if (category) {
            filteredFurniture = filteredFurniture.filter(f => f.Category && f.Category.toLowerCase().includes(category.toLowerCase()));
        }
        // 按类型筛选
        if (type) {
            filteredFurniture = filteredFurniture.filter(f => f.Type && f.Type.toLowerCase().includes(type.toLowerCase()));
        }
        // 按稀有度筛选
        if (rarity !== undefined) {
        ...
        // 按标签筛选
        if (tags) {
            filteredFurniture = filteredFurniture.filter(f => f.Tags && f.Tags.some(tag => tag.toLowerCase().includes(tags.toLowerCase())));
        }
        // 智能搜索
        if (search) {
            filteredFurniture = this.smartSearch(filteredFurniture, search, ['Name', 'Category', 'Type', 'Tags', 'Description']);
        }
        // 限制结果数量
    ...
    async getEnemiesEnhanced(options = {}) {
        const { language = 'cn', search, type, rank, armorType, bulletType, terrain, limit = 20, detailed = false } = options;
        const data = await this.fetchData(`${language}/enemies.json`);
        // 处理数据格式
        let enemies;
        if (data && typeof data === 'object' && !Array.isArray(data)) {
            enemies = Object.values(data);
        }
        else {
        ...
        // 按类型筛选
        if (type) {
            filteredEnemies = filteredEnemies.filter(e => e.Type && e.Type.toLowerCase().includes(type.toLowerCase()));
        }
        // 按等级筛选
        if (rank) {
            filteredEnemies = filteredEnemies.filter(e => e.Rank && e.Rank.toLowerCase().includes(rank.toLowerCase()));
        }
        // 按护甲类型筛选
        if (armorType) {
            filteredEnemies = filteredEnemies.filter(e => e.ArmorType && e.ArmorType.toLowerCase().includes(armorType.toLowerCase()));
        }
        // 按子弹类型筛选
        if (bulletType) {
            filteredEnemies = filteredEnemies.filter(e => e.BulletType && e.BulletType.toLowerCase().includes(bulletType.toLowerCase()));
        }
        // 按地形筛选
        if (terrain) {
            filteredEnemies = filteredEnemies.filter(e => e.Terrain && e.Terrain.some(t => t.toLowerCase().includes(terrain.toLowerCase())));
        }
        // 智能搜索
        if (search) {
            filteredEnemies = this.smartSearch(filteredEnemies, search, ['Name', 'Type', 'Rank', 'ArmorType', 'BulletType', 'WeaponType']);
        }
        // 限制结果数量
    ...
    // 查找角色变体
    async findStudentVariants(name, language = 'cn', includeOriginal = true) {
    ...
    // 批量获取学生头像
    async getMultipleStudentAvatars(studentIds, language = 'cn', avatarType = 'portrait') {
        const results = [];
    ...
    // 批量获取学生语音
    async getMultipleStudentVoices(studentIds, language = 'cn', voiceType = 'all') {
        const results = [];
        try {
            ...
            for (const studentId of studentIds) {
                try {
                    ...
                    if (targetStudent && voiceData && voiceData[studentId]) {
                        ...
                        // 根据语音类型筛选
                        if (voiceType !== 'all') {
                            filteredVoices = studentVoices.filter((voice) => {
...
class BlueArchiveMCPServer {
    ...
    async handleGetStudents(args) {
        const { language, search, limit, detailed, school, starGrade, role } = args;
...
---`;
            ...
            // 简要模式 - 只显示核心信息
            result = students.map(student => `${student.Name} (${student.School}) - ${student.TacticRole} - ⭐${student.StarGrade}`).join('\n');
        }
        return {
            content: [
                {
    ...
    async handleGetStudentByName(args) {
...
弹药类型: ${student.BulletType || '未知'}
    ...
    async handleGetStudentInfo(args) {
    ...
    async handleGetRaids(args) {
        ...
        if (search) {
            ...
            filteredRaids = raids.filter(raid => raid.Name?.toLowerCase().includes(searchLower));
        }
...
---`;
            ...
            result = filteredRaids.map(raid => `${raid.Name || '未知'} (ID: ${raid.Id}) - 等级 ${raid.Level || '未知'} - ${raid.Terrain || '未知地形'}`).join('\n');
        }
        return {
            content: [
                {
    ...
    async handleGetEquipment(args) {
        ...
        if (category) {
            ...
            filteredEquipment = filteredEquipment.filter(eq => eq.Category?.toLowerCase().includes(categoryLower));
        }
        // 按等级过滤
        if (tier) {
        ...
        // 应用数据精简
        const processedEquipment = limitedEquipment.map(eq => this.client['simplifyEquipmentData'](eq, normalizedDetailed));
        let result;
        if (normalizedDetailed) {
...
---`;
            ...
            result = processedEquipment.map(eq => `${eq.Name || '未知'} (T${eq.Tier || '?'}) - ${eq.Category || '未知类别'}`).join('\n');
        }
    ...
    async handleGetGameConfig(args) {
...
地区: ${config.region || '未知'}
    ...
    async handleGetStages(args) {
        const { language, search, area, chapter, difficulty, limit, detailed } = args;
        const normalizedLanguage = ParameterHandler.normalizeLanguage(language);
        const normalizedLimit = ParameterHandler.normalizeLimit(limit, 20);
        const normalizedDetailed = ParameterHandler.normalizeDetailed(detailed);
        ...
        processedStages.forEach((stage, index) => {
            ...
            if (stage.Area)
                result += `   区域: ${stage.Area}\n`;
            if (stage.Chapter)
                result += `   章节: ${stage.Chapter}\n`;
            if (stage.Difficulty)
                result += `   难度: ${stage.Difficulty}\n`;
            if (stage.StageNumber)
                result += `   关卡编号: ${stage.StageNumber}\n`;
            if (stage.APCost)
                result += `   AP消耗: ${stage.APCost}\n`;
            if (stage.Terrain)
                result += `   地形: ${stage.Terrain}\n`;
            if (stage.RecommendLevel)
                result += `   推荐等级: ${stage.RecommendLevel}\n`;
            if (normalizedDetailed && stage.DropList && stage.DropList.length > 0) {
                result += `   掉落物品: ${stage.DropList.map((drop) => drop.Name || drop.Id).join(', ')}\n`;
            }
            if (normalizedDetailed && stage.EnemyList && stage.EnemyList.length > 0) {
                result += `   敌人列表: ${stage.EnemyList.map((enemy) => enemy.Name || enemy.Id).join(', ')}\n`;
            }
            result += '\n';
        });
        return {
    ...
    async handleGetEnemies(args) {
        const { language, search, type, rank, armorType, bulletType, terrain, limit, detailed } = args;
        const normalizedLanguage = ParameterHandler.normalizeLanguage(language);
        const normalizedLimit = ParameterHandler.normalizeLimit(limit, 20);
        const normalizedDetailed = ParameterHandler.normalizeDetailed(detailed);
    ...
    async handleGetStudentAvatar(args) {
        ...
        try {
            ...
            const typeName = avatarTypeNames[avatarType?.toLowerCase()] || avatarType || 'portrait';
            // 只支持markdown格式输出
            return {
                content: [
                    {
    ...
    async handleGetStudentVoice(args) {
    ...
    // 新增的处理函数
    async handleFindStudentVariants(args) {
        ...
        try {
            ...
            if (normalizedFormat === 'markdown' || normalizedFormat === 'md') {
                result = `# ${name} 的角色变体\n\n`;
                result += variants.map(variant => `- **${variant.名称 || variant.Name}** (ID: ${variant.ID || variant.Id}) - 相似度: ${(variant.similarity * 100).toFixed(1)}%`).join('\n');
            }
            else {
                result = `${name} 的角色变体：\n\n`;
                result += variants.map(variant => `${variant.名称 || variant.Name} (ID: ${variant.ID || variant.Id}) - 相似度: ${(variant.similarity * 100).toFixed(1)}%`).join('\n');
    ...
    async handleGetMultipleStudentAvatars(args) {
        ...
        try {
            ...
            if (normalizedFormat === 'markdown' || normalizedFormat === 'md') {
                ...
                result = `学生头像合集：\n\n`;
                result += avatars.map(avatar => `${avatar.name}: ${avatar.url}`).join('\n');
            }
            return {
                content: [
                    {
    ...
    async handleGetMultipleStudentVoices(args) {
