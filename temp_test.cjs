...
class SchaleDBClient {
    ...
    async getStudentsEnhanced(options = {}) {
        const { language = 'cn', search, limit = 20, detailed = false, school, starGrade, role } = options;
        const suffix = '.json';
    ...
    async getStudents(language = 'cn', compressed = false) {
...
class BlueArchiveMCPServer {
    ...
    async handleGetStudents(args) {
        const { language = 'cn', search, limit = 20, detailed = false, school, starGrade, role } = args;
        // 使用增强的方法
...
---`;
    ...
    async handleGetEquipment(args) {
        ...
        // 应用数据精简
        const processedEquipment = limitedEquipment.map(eq => this.client['simplifyEquipmentData'](eq, detailed));
        let result;
        if (detailed) {
...
地区: ${config.region || '未知'}
    ...
    async handleGetEnemies(args) {
        const { language = 'cn', search, type, rank, armorType, bulletType, terrain, limit = 20, detailed = false } = args;
    ...
    async handleFindStudentVariants(args) {
        ...
        try {
            ...
            if (format === 'markdown' || format === 'md') {
                result = `# ${name} 的角色变体\n\n`;
                result += variants.map(variant => `- **${variant.name}** (ID: ${variant.id}) - 相似度: ${(variant.similarity * 100).toFixed(1)}%`).join('\n');
            }
            else {
                result = `${name} 的角色变体：\n\n`;
                result += variants.map(variant => `${variant.name} (ID: ${variant.id}) - 相似度: ${(variant.similarity * 100).toFixed(1)}%`).join('\n');
